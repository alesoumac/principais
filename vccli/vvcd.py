...
def get_json_imagem(img_filename, use_local_server, dir_work):
    ...
    num_matches = 0
    num_matches_maximo = len(expected_values)
    print(f'Numero de Campos Esperados = {num_matches_maximo}')
    ...
    return num_matches, num_matches_maximo, detected_objs
...
def verifyArquivo(nomearq, dir_work):
    ...
    matches, max_matches, detectado = get_json_imagem(nomearq, USE_LOCAL_SERVER, dir_work)
    ...
    for root, _, files in os.walk(pasta):
        ...
        for fi in filessrt:
            ...
            if ex[1:].lower() in listaExt and '.temp__' not in root:
                matches, max_matches = verifyArquivo(os.path.join(root,fi), dir_work)
                sum_matches += matches
...
def main():
    ...
    for nomearq in arquivos:
        if os.path.isdir(nomearq):
            ...
            matches, max_matches = verifyArquivo(nomearq, ME.DIR_TEMP)

        sum_matches += matches
        sum_max_matches += max_matches

    if sum_max_matches > 0:
        print("Percentual de acerto = {:5.2f}%".format(float(sum_matches)/sum_max_matches*100.0))
    ...
    print("Número de imagens processadas = {}".format(CONTADOR_ARQUIVOS))
    print("Tempo médio de processamento por imagem = {}".format(tempo_processamento / CONTADOR_ARQUIVOS))
    
    NCPFS = CPF_OK + CPF_NOK
    if NCPFS > 0:
        print("CPFs testados: {} | CPFs com Erro: {} | Taxa de Acerto de CPF: {:5.2f}%".format(
            NCPFS, CPF_NOK, float(CPF_OK)*100/(NCPFS)))
    else:
        print("Não foram testados CPFs nessa imagem ou grupo de imagens")
