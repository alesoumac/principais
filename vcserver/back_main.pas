...
uses
  ...
  fpjson, jsonparser, Grids, Menus, process;
...
type
  ...
  private
    ...
    PathProg : string;
    ImagesDir : string;
    ApiAddress,ApiDetectOCR,ApiDetect : string;
...
begin
  ...
  HTTPObj.AddHeader('env_auth', '2');
end;
{
procedure TfrmMain.HttpClientGetSocketHandler(Sender: TObject;
  const UseSSL: Boolean; out AHandler: TSocketHandler);
begin
...
begin
  ...
  try
    ...
    S := MakeURLParam('image',S)
         +'&'+MakeURLParam('get_img_flg','True')
         +'&'+MakeURLParam('resize',sResize)
         +'&'+MakeURLParam('align',sRotation)
         +'&'+MakeURLParam('crop_height','120')
         +'&'+MakeURLParam('ocr_kind','2')
         ;
            ...
            begin
              ...
              then begin
                ...
                begin
                  ...
                  grdValores.Cells[4,i] := GetValueJSON(Valores,Chave);
                end;
              end;
              MatchSum := 0;
              NumMatch := 0;
              ...
              then begin
                try
                  ...
                  begin
                    ...
                    Ymax := Ymin + Hei;
                    ObjName := GetValueJSON(RLI,'obj_name');
                    OcrText := GetValueJSON(RLI,'adjusted_ocr');
...
begin
  ...
  try
    ...
    ImagePanelWidth := ini.ReadFloat('Geometry','ImagePanelWidth',1/3);
...
begin
  ...
  try
    ...
    // -------- APIServer
    ini.WriteString('APIServer','BaseAddress',ApiAddress);
    ini.WriteString('APIServer','Detect',ApiDetect);
    ini.WriteString('APIServer','DetectOCR',ApiDetectOCR);
...
begin
  ...
  try
    ...
    // -------- APIServer
    ApiAddress   := ini.ReadString('APIServer','BaseAddress','http://localhost:8081/v1/');
...
begin
  ...
  Redimensionando := True;
  try
    lstArquivos.Height := Round(FileListHeight * Height);
    panImagem.Width := Round(ImagePanelWidth * Width);
    panJSON.Height := Round(JSONPanelHeight * Height);
  finally
    Redimensionando := False;
  end;
end;
  ...
  then begin
    ...
    then begin
      ...
      begin
        ...
        grdValores.Cells[4,i] := GetValueJSON(Valores,ObjName);
      end;
    end
    else begin
      grpDoc.ItemIndex := -1;
...
begin
  ...
  FileListHeight := lstArquivos.Height / Height;
end;
...
begin
  if not Comecou or Redimensionando then Exit;
  JSONPanelHeight := panJSON.Height / Height;
end;
